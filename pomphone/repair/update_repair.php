...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($status) && !empty($tel)) {
        notifyRepairStatus($pdo, $tel, $status, $repair_id, $customer_price ?? null);
    }
    ...
    <div class="container-fluid">
      ...
      <form method="POST">
        <div class="form-group">
          ...
          <input type="text" class="form-control" value="<?= safe_text($repair['customer_name']) ?>" disabled>
        </div>
        <div class="form-group">
          <label>อาการเสีย:</label>
          <textarea class="form-control" rows="2" disabled><?= safe_text($repair['issue_desc']) ?></textarea>
        </div>
        <div class="form-group">
