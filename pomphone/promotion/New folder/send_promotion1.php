...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($template_id && $template_id !== '') {
        ...
        $tpl = $stmt->fetch();
        if ($tpl) {
            $message = $tpl['text'] ?? $message;
            $image_url = $tpl['image_url'] ?? $image_url;
            $flex_json = $tpl['flex_json'] ?? $flex_json;
        }
    }

    $sql = "SELECT line_user_id FROM line_users lu JOIN customer_account ca ON lu.cua_id = ca.cua_id WHERE lu.user_type = 'customer'";
    $params = [];
    ...
    if ($message || $image_url || $flex_json) {
        ...
        if ($schedule !== '') {
            $pdo->prepare("INSERT INTO promotion_logs (template_id, message, flex_json, schedule, recipients) VALUES (?, ?, ?, ?, ?)")
                ->execute([$template_id ?: null, $message, $flex_json, $schedule, count($recipients)]);
            $msg = "\ud83d\uddd3\ufe0f \u0e15\u0e31\u0e49\u0e07\u0e40\u0e27\u0e25\u0e32\u0e2a\u0e48\u0e07\u0e41\u0e25\u0e49\u0e27 \u0e08\u0e33\u0e19\u0e27\u0e19 " . count($recipients) . " \u0e23\u0e32\u0e22";
        } else {
            foreach ($recipients as $line_id) {
                pushMessage($line_id, $content);
                $log_id = $pdo->lastInsertId();

                $insert = $pdo->prepare("INSERT INTO promotion_recipients (log_id, line_user_id, cua_id) VALUES (?, ?, ?)");

                foreach ($recipients as $line_id) {
                    // หา cua_id
                    $stmt = $pdo->prepare("SELECT cua_id FROM line_users WHERE line_user_id = ? LIMIT 1");
                    $stmt->execute([$line_id]);
                    $cua_id = $stmt->fetchColumn();

                    $insert->execute([$log_id, $line_id, $cua_id ?: null]);
                }
            }
            $pdo->prepare("INSERT INTO promotion_logs (template_id, message, flex_json, schedule, recipients) VALUES (?, ?, ?, NOW(), ?)")
                ->execute([$template_id ?: null, $message, $flex_json, count($recipients)]);
            $msg = "\u2705 \u0e2a\u0e48\u0e07\u0e02\u0e49\u0e2d\u0e04\u0e27\u0e32\u0e21\u0e2a\u0e33\u0e40\u0e23\u0e47\u0e08\u0e16\u0e36\u0e07\u0e25\u0e39\u0e01\u0e04\u0e49\u0e32 " . count($recipients) . " \u0e23\u0e32\u0e22";
        }
    } else {
...
<script>
        ...
        function loadTemplatePreview(id) {
          ...
          let html = '';
          if (t.text) html += `<p><strong>ข้อความ:</strong> ${t.text}</p>`;
