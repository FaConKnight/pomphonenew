...
foreach ($sales as $s) {
    ...
    $stmt3 = $pdo->prepare("SELECT si.qty, si.price, si.cost_price, si.item_discount, p.name, p.category_id FROM sale_items si LEFT JOIN products p ON si.product_id = p.id WHERE sale_id = ?");
    $stmt3->execute([$s['id']]);
    foreach ($stmt3->fetchAll() as $i) {
        $cost = $i['qty'] * $i['cost_price'];
        $profit = (($i['price'] - $i['item_discount']) - $i['cost_price']) * $i['qty'];
        $total_cost += $cost;
        $total_profit += $profit;
        ...
        $by_product[$pname]['amount'] += $i['qty'] * ($i['price'] - $i['item_discount']);
...
<div class="main-content">
    <div class="section__content section__content--p30">
        <div class="container-fluid">
            <h3 class="mb-4">รายงานยอดขาย(หักส่วนลด) <a href="sale_report_nondiscount.php" style="font-size: 10px" >แบบไม่หักส่วนลด</a></h3>
