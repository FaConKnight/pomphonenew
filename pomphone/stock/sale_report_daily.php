...
$sql = "SELECT s.id, s.sale_time, si.item_discount, e.emd_name, si.qty, si.price, si.imei, si.cost_price, p.name
        FROM sale s
...
<div class="main-content">
    <div class="section__content section__content--p30">
        <div class="container-fluid">
            <h3 class="mb-4">รายการสินค้าที่ขายเมื่อวันที่ <?= safe_text($date) ?></h3>

            <form method="GET" class="form-inline mb-3">
                <input type="hidden" name="date" value="<?= safe_text($date) ?>">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <?php foreach ($items as $row): 
                        $subtotal = $row['qty'] * ($row['price'] - $row['item_discount']);
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?= safe_text($row['imei']) ?></td>
                            <td><?= $row['qty'] ?></td>
                            <td><?= number_format(($row['price'] - $row['item_discount'])?? 0, 2) ?></td>
                            <?php if ($show_cost): ?>
                                <td><?= number_format($row['cost_price']?? 0, 2) ?></td>
                                <td><?= number_format(($row['price'] - $row['item_discount']) - $row['cost_price']?? 0, 2) ?></td>
                            <?php endif; ?>
                            <td><?= number_format($subtotal?? 0, 2) ?></td>
                            <td><?= safe_text($row['emd_name']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
                <tfoot class="font-weight-bold">
                    ...
                    <tr>
                        ...
                        <td colspan="2"><?= number_format($total_cost?? 0, 2) ?> บาท</td>
                    </tr>
                    <tr>
                        <td colspan="7" class="text-right">รวมกำไร:</td>
                        <td colspan="2"><?= number_format($total_profit?? 0, 2) ?> บาท</td>
                    </tr>
                    <?php endif; ?>
                </tfoot>
            </table>
