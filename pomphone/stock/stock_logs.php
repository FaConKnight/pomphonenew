    ...
    <div class="section__content section__content--p30">
      <div class="container-fluid">
        ...
        <form method="GET" class="form-inline mb-3">
          <input type="text" name="search" placeholder="ค้นหา..." value="<?= safe_text($search) ?>" class="form-control mr-2">
        ...
        <table class="table table-striped table-bordered">
          ...
          <tbody>
            <?php foreach ($logs as $i => $log): ?>
              <tr>
                <td><?= $i + 1 ?></td>
                <td><?= date('d/m/Y H:i', strtotime(safe_date($log['created_at']))) ?></td>
                <td><?= safe_text($log['product_name'] ?? '-') ?></td>
                <td><?= safe_text($log['imei1'] ?? $log['sku'] ?? '-') ?></td>
                <td><?= $actions[$log['action']] ?? $log['action'] ?></td>
                <td><?= safe_text($log['em_username'] ?? '-') ?></td>
                <td><?= safe_text($log['remark']) ?></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
