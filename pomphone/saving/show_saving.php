        ...
        <table class="table table-striped table-bordered">
          ...
          <tbody>
            <?php foreach ($savings as $i => $s): ?>
              <tr>
                <td><?= $i + 1 ?></td>
                <td><?= safe_text($s['saving_ref']) ?></td>
                <td><?= safe_text($s['cua_name'] . ' ' . $s['cua_lastname']) ?></td>
                <td><?= safe_text($s['product_name']) ?></td>
                <td><?= number_format($s['total_price']?? 0, 2) ?></td>
                <td><?= number_format($s['paid_amount']?? 0, 2) ?></td>
                <td><?= safe_text($s['status']) ?></td>
                <td><?= date('d/m/Y H:i', strtotime(safe_date($s['created_at']))) ?></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
